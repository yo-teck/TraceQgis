...
def test_get_actions(tmp_path):
    parser = create_parser_for_test(tmp_path)
    names = set(parser.get_actions())
    expected = {"LOAD-TRUCK", "UNLOAD-TRUCK", "DRIVE-TRUCK"}
...
def test_execution_sequence(tmp_path):
    parser = create_parser_for_test(tmp_path)
    seq = parser.get_execution_sequence()
    assert seq == [
...
def test_get_predicates_and_parameters(tmp_path):
    parser = create_parser_for_test(tmp_path)
    preds = parser.get_predicates()
    # Vérifie que chaque prédicat existe avec bons paramètres et types
    assert "in-city" in preds
    assert preds["in-city"] == [('?loc', 'place'), ('?city', 'city')]
    assert "at" in preds
    assert preds["at"] == [('?obj', 'physobj'), ('?loc', 'place')]

    # Vérification des paramètres d'actions
    action_params = parser.get_action_parameters()
    assert "LOAD-TRUCK" in action_params
    assert action_params["LOAD-TRUCK"] == [('?pkg', 'package'), ('?truck', 'truck'), ('?loc', 'place')]
    assert action_params["DRIVE-TRUCK"] == [('?truck', 'truck'), ('?from', 'place'), ('?to', 'place'), ('?city', 'city')]
